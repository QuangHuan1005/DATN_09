    ...
    <div class="container-xxl">
        <div class="row">
            <div class="col-xl-8 col-lg-10 mx-auto">
                <form action="{{ route('admin.attributes.colors.update', $color->id) }}" method="post">
                    @csrf
                    @method('PUT')
                    <div class="card">
                                ...
                                <div class="col-lg-6">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control @error('status') is-invalid @enderror" 
                                            ...
                                            <option value="active" {{ old('status', $color->status) == 'active' ? 'selected' : '' }}>Hoạt động</option>
                                            <option value="inactive" {{ old('status', $color->status) == 'inactive' ? 'selected' : '' }}>Không hoạt động</option>
                                        </select>
                                        @error('status')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                ...
                                <div class="col-lg-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" 
                                            value="{{ $color->variants()->count() }} biến thể" 
                                            readonly disabled>
                                    </div>
                                </div>
                                <div class="col-lg-12">
                                    <div class="mb-0">
                                        ...
                                        <textarea class="form-control bg-light-subtle" name="description" 
                                            ...
                                            placeholder="Mô tả về màu sắc (tùy chọn)">{{ old('description', $color->description) }}</textarea>
                                        @error('description')
                                            <div class="invalid-feedback d-block">{{ $message }}</div>
                                        @enderror
                                    </div>
