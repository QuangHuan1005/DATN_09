            ...
            <div class="col-md-6 text-end">
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-end">
                        <a href="{{ route('admin.orders.index') }}" class="dropdown-item">Tất cả trạng thái</a>
                        @foreach ($statuses as $status)
                            <a href="{{ route('admin.orders.index', ['status' => $status->id]) }}" class="dropdown-item">
                                {{ $status->name }}
                            </a>
                        @endforeach
                    </div>
                        ...
                        <tbody>
                            @forelse($orders as $order)
                                @php
                                    ...
                                    // Màu sắc cho 3 trạng thái thanh toán
                                    $paymentColors = [
                                        1 => 'badge border border-warning text-warning',   // Chưa thanh toán
                                        2 => 'badge border border-success text-success',   // Đã thanh toán
                                        3 => 'badge border border-info text-info',         // Hoàn tiền
                                    ];
                                    $paymentColor = $paymentColors[$order->payment_status_id] ?? 'badge border border-secondary text-secondary';
