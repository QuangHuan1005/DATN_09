...
@section('content')
    ...
    <style>
        ...
        .cart-summary tbody tr:nth-child(2) {
            background: #fafafa;
        }
        ...
        /* dùng biến cũ để đổi màu toàn cục */
        ...
        .cart-actions {
            width: 100%;
            /* = max-width của .cart-summary bên phải */
            margin-top: 16px;
        }

        .cart-actions .btn-update {
            display: block;
            /* giống a.btn-checkout */
            width: 100%;
            /* chiếm toàn bộ .cart-actions (520px) */
            padding: 12px 16px;
            /* khớp padding với nút checkout */
            border-radius: 6px;
            /* khớp bo góc */
        }
        ...
        .js-check {
            accent-color: #000;
            /* nền/tô đen khi checked, dấu tick trắng */
            width: 16px;
            height: 16px;
            border-radius: 4px;
            /* bo nhẹ cho đẹp */
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            <div class="row align-items-start g-4">
                                                    ...
                                                    <tr data-variant="{{ $row['variant_id'] }}">
                                                        <td style="text-align:center">
                                                            <input type="checkbox" class="js-check"
                                                                data-variant="{{ $row['variant_id'] }}" checked />
                                                        </td>
                                ...
                                giá đơn vị --}}
                                                        ...
                                                        <td class="pro-subtotal">
                                                            <span class="fw-semibold"
                                                                id="line-total-{{ $row['variant_id'] }}">{{ number_format($lineTotal) }}đ</span>
                                                        </td>

                                                        <td class="pro-remove text-center">
                                                            <form action="{{ route('cart.remove', $row['variant_id']) }}"
                                                                method="POST" class="js-remove-form">
                                                                @csrf @method('DELETE')
                                                                <button type="button" class="btn-remove js-remove"
                                                                    title="Xóa"
                                        ...
                                        <table class="table table-borderless mb-0">
                                            ...
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td class="text-end" id="sum-products">
                                                        {{ number_format($tongGioHang) }}đ</td>
                                                </tr>
                                                <tr class="total">
                                                    <td>Tổng thanh toán</td>
                                                    <td class="text-end" id="sum-grand">{{ number_format($tongGioHang) }}đ
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
                                ...
                                <a href="{{ route('checkout.index') }}" class="btn-checkout mt-3 w-100"
    ...
    <script>
        ...
        document.querySelectorAll('a.btn-checkout').forEach(a => {
            a.addEventListener('click', function(e) {
                const chosen = [...document.querySelectorAll('.js-check:checked')].map(x => x.dataset
                    .variant);
                if (chosen.length === 0) {
                    Swal.fire({
                        icon: 'warning',
