...
class OrderController extends Controller
{
    ...
    public function show($id)
    {
        ...
        // Dùng giá trị trực tiếp từ database để đảm bảo nhất quán
        $calc_subtotal = (float)$order->subtotal;
        $calc_discount = (float)$order->discount;
        $calc_total    = (float)$order->total_amount;
        // Tính shipping fee từ database: giả sử có cột shipping_fee, nếu không thì tính
        $calc_shipping_fee = isset($order->shipping_fee) ? (float)$order->shipping_fee : max(0, $calc_total - $calc_subtotal + $calc_discount);
    ...
    // Chi tiết đơn hàng đã hoàn tiền
    public function showRefunded($id)
    {
        $order = Order::query()
            ->with([
                'status',
        ...
        // Kiểm tra xem đơn này có phải đã hoàn tiền không
        $hasCompletedReturn = \App\Models\OrderReturn::where('order_id', $order->id)
                                                     ->where('status', 'refunded')
                                                     ->exists();

        if (!$hasCompletedReturn) {
            return redirect()->route('orders.show', $id)->with('error', 'Đơn hàng này chưa được hoàn tiền.');
        }
        ...
        $lines = $order->details->map(function ($d) {
            ...
            return (object) [
                ...
                'product_name' => $variant?->product?->name ?? 'Sản phẩm không tồn tại',
                'variant_text' => $variant ? trim(($variant->color?->name ?? '') . ' ' . ($variant->size?->name ?? '')) : '',
                'image' => $variant?->product?->photoAlbums?->first()?->image,
                'unit_price' => $d->price,
                'qty' => $d->quantity,
                'line_total' => $d->total_price,
                'eta' => $d->estimated_delivery,
            ];
        });
        ...
        return view('orders.refunded.show', compact(
            'order',
            'lines',
            'calc_subtotal',
            'calc_shipping_fee',
