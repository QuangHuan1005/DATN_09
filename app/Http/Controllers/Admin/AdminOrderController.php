...
class AdminOrderController extends Controller
{
    ...
    public function update(Request $request, $id, InventoryService $inv)
    {
        ...
        // Lấy payment_method_id từ relationship payment
        $paymentMethodId = $order->payment ? (int)$order->payment->payment_method_id : null;

        // Khi chuyển sang "Đã giao hàng", tự động chuyển sang "Đã thanh toán"
        // Áp dụng cho cả COD và Online payment
        if ($newStatus === self::STATUS_DELIVERED) {
            // Nếu chưa thanh toán, chuyển sang đã thanh toán
            if ((int)$order->payment_status_id === $PAYMENT_STATUS_UNPAID) {
                $order->payment_status_id = $PAYMENT_STATUS_PAID;
            }
        }

        // Khi chuyển sang "Hoàn thành", đảm bảo đã thanh toán
        if ($newStatus === self::STATUS_DONE) {
            if ((int)$order->payment_status_id !== $PAYMENT_STATUS_PAID) {
                $order->payment_status_id = $PAYMENT_STATUS_PAID;
            }
        }
